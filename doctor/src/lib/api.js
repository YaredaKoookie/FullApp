...
const appointments = {
  ...
  complete: (id) => api.post(`/doctors/appointments/${id}/complete`),
};
...
const doctor = {
  ...
  getStats: async (timeRange = 'week') => {
    const response = await api.get(`/doctors/stats?timeRange=${timeRange}`);
    return response.data;
  },
  getActivities: async () => {
    const response = await api.get('/doctors/activities');
    return response.data;
  }
};
...
export const adminAPI = {
  ...
  appointments: {
    getAll: async (params = {}) => {
      ...
      const response = await api.get(`/doctors/appointments?${queryString}`);
      return response;
    },
    getStats: async () => {
    ...
    accept: async (id) => {
      const response = await api.post(`/doctors/appointments/${id}/accept`);
      return response.data;
    },
    reject: async (id, note) => {
      const response = await api.post(`/doctors/appointments/${id}/reject`, { note });
      return response.data;
    },
    complete: async (id) => {
      const response = await api.post(`/doctors/appointments/${id}/complete`);
      return response.data;
    },
  },
};
...
export const medicalRecordAPI = {
  create: async (patientId, data) => {
    const response = await api.post(`/doctors/medical-records`, {
      patient: patientId,
      ...data
    });
    return response;
  ...
  getPatientRecords: async (patientId) => {
    const response = await api.get(`/doctors/medical-records/patient/${patientId}`);
    return response;
  },
  getRecordById: async (recordId) => {
    const response = await api.get(`/doctors/medical-records/${recordId}`);
    return response;
  },
  addClinicalNote: async (recordId, note) => {
    const response = await api.post(`/doctors/medical-records/${recordId}/clinical-notes`, { note });
    return response;
  },
  addDiagnosis: async (recordId, diagnosis) => {
    const response = await api.post(`/doctors/medical-records/${recordId}/diagnoses`, diagnosis);
    return response;
  }
}; 