...
import { Calendar, Search, Filter, ChevronDown, CheckCircle2 } from "lucide-react";
...
const AppointmentsContent = () => {
  ...
  const completeAppointmentMutation = useMutation({
    mutationFn: (id) => adminAPI.appointments.complete(id),
    onSuccess: () => {
      queryClient.invalidateQueries(["appointments"]);
      queryClient.invalidateQueries(["appointmentStats"]);
    ...
    onError: (error) => {
      toast.error(
        error.response?.data?.message || "Failed to mark appointment as completed"
      );
    },
  });
                    ...
                    <td className="px-6 py-4 whitespace-nowrap text-sm font-medium">
                      ...
                      {appointment.status === "confirmed" && (
                        <div className="flex space-x-2">
                          {(() => {
                            const now = Date.now();
                            const startTime = new Date(appointment.slot.start).getTime();
                            const endTime = new Date(appointment.slot.end).getTime();
                            const fiveMinutesBeforeStart = startTime - (5 * 60 * 1000); // 5 minutes in milliseconds

                            if (now >= fiveMinutesBeforeStart && now <= endTime) {
                              return (
                                <Link
                              ...
                              return (
                                <>
                                  <button
                                    onClick={() => {
                                      if (window.confirm("Are you sure you want to mark this appointment as completed?")) {
                                        completeAppointmentMutation.mutate(appointment._id);
                                      }
                                    }}
                                  ...
                                  <div className="text-xs text-gray-500">
                                    Ended {new Date(appointment.slot.end).toLocaleTimeString()}
                                  </div>
                                </>
                              );
                            } else {
                              const minutesToStart = Math.ceil((startTime - now) / (60 * 1000));
                              return (
                                <div className="text-xs text-gray-500">
