...
const SecurityPage = () => {
  ...
  const passwordMutation = useMutation({
    mutationFn: (data) => {
      const endpoint = user?.isPasswordSet
      ...
      return adminAPI.post(endpoint, payload);
    },
    onSuccess: () => {
      toast.success(
        `Password ${user?.isPasswordSet ? "changed" : "set"} successfully`
      );
      reset();
  ...
  const logoutMutation = useMutation({
    mutationFn: (sessionId) =>
      sessionId
        ? adminAPI.delete(`/auth/sessions/${sessionId}`)
        : adminAPI.delete("/auth/sessions"),
    onSuccess: () => {
  ...
  const getDeviceInfo = (session) => {
    ...
    const parts = [device.browser, device.os, device.model].filter(Boolean);
                    ...
                    <div className="space-y-3">
                      {Array.isArray(sessions) &&
                        sessions.slice(0, 3).map((session) => {
                          const { name, icon, isCurrent, createdAt, location } =
                ...
                >
                  <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
                    ...
                    <div>
                      <label className="block text-sm font-medium text-gray-700 mb-1">
                        Confirm{" "}
                        {user?.isPasswordSet ? "New Password" : "Password"}
                      </label>
                      <input
                        type="password"
                        ...
                        return (
                          ...
                          >
                            ...
                            {!isCurrent && (
                              <button
                                onClick={() =>
                                  logoutMutation.mutate(session._id)
                                }
                                disabled={logoutMutation.isPending}
          ...
          <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
            <div className="bg-white rounded-lg p-6 max-w-md w-full mx-4">
              ...
              <p className="text-gray-600 mb-6">
                You are about to logout from this device. You will be redirected
                to the home page.
              </p>
